...
import type { ButtonInteraction, ModalSubmitInteraction, TextInputComponent } from "discord.js";
...
export function registerSelectTimeStep() {
  ...
  registerUIAction("events.create.selectDay", {
    ...
    handler: async (interaction: ButtonInteraction, ctx: TraceContext) => {
      ...
      const [day, month] = interaction.customId.split("|")[1].split("-").map(Number);
  ...
  registerUIAction("events.create.selectTime.submit", {
    ...
    handler: async (interaction: ModalSubmitInteraction, ctx: TraceContext) => {
      ...
      // Możemy użyć getEventDateUTC(day, month, hours, minutes)

      await interaction.reply({
        content: `You set event date: ${day}/${month} ${hours}:${minutes} UTC`,
        ephemeral: true,
      });
    },
  });
