...
export async function handleInteraction(ctx: TraceContext, customId: string): Promise<ActionResult> {
  ...
  if (isExpired(entry)) { store.delete(customId); return { type: "reply", content: "⏳ Interaction expired." }; }

  const action = actions.get(entry.action);
  if (!action) return { type: "reply", content: "❌ Action not found." };

  try { const result = await action.execute(ctx, entry.state); return result; } 
  catch (err) { return { type: "reply", content: "❌ Something went wrong." }; }
}