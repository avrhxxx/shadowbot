...
export const IDS = {
  BUTTONS: { ADD: "absence_add", REMOVE: "absence_remove", SHOW_LIST: "absence_list", SETTINGS: "absence_settings" },
...
export async function handleAbsenceInteraction(interaction: Interaction<CacheType>) {
  try {
    if (interaction.isButton()) {
      const handler = BUTTON_HANDLERS[interaction.customId]; if (!handler) return; return await handler(interaction);
    }
    if (interaction.isStringSelectMenu()) {
      const handler = SELECT_HANDLERS[interaction.customId]; if (!handler) return; return await handler(interaction);
    }
    if (interaction.isModalSubmit()) return await handleModal(interaction);
  } catch (error) {
    console.error("Error handling absence interaction:", error);
    if (interaction.isRepliable()) {
      if (interaction.replied || interaction.deferred) await interaction.followUp({ content: "❌ An error occurred while processing this interaction.", ephemeral: true });
      else await interaction.reply({ content: "❌ An error occurred while processing this interaction.", ephemeral: true });
    }
  }
}